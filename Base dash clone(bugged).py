...
while True: #Game loop-------------------------------------------------------------
    ...
    if random.randint(1, 100) <= 40: #Random chance to create a new platform
        platforms.append(Platform())
        print("apeending platform!")
        
    #remove platforms that have moved off screen
    for platform in platforms:
        if platform.x + platform.width < 200: #check if the platform is off the screen
            platforms.pop(platform) #fun fact: if this was C++, we'd need to use a destructor
